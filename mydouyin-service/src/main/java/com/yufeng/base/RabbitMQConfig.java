...
public class RabbitMQConfig {
    ...
    /**
     * 5.绑定交换机和队列(其实就是设定路由规则: 规定生产者需要发送什么key，才能到指定的队列)
    ...
    public Binding bindingSysMsg() {
        return BindingBuilder
                ...
                .with("sys.msg.*")     // 路由键(虽然这里只定义了一个key，但实际上生产者可以发送多个不同的key，比如 sys.msg.login、sys.msg.logout 等) 底层理解: 这就是所谓的路由规则，生产者发布信息的时候，只有使用sys.msg.*这个key，才会被QUEUE_SYS_MSG这个队列所接收
                .noargs();           // 不需要额外参数
